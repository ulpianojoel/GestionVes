...
namespace Ves.DAL.Repositories
{
    public sealed class AuditRepository
    {
        ...
        public AuditRepository(string logConnectionString) => _cs = logConnectionString;

        public async Task LogLoginAsync(string usuario, bool exitoso, string? ip = null, string? extra = null)
        {
            const string sql = "INSERT INTO VesLog.dbo.LoginEvents (Usuario, Exitoso, Ip, Extra) VALUES (@u,@e,@ip,@x);";
        ...
        {
            const string sql = "INSERT INTO VesLog.dbo.BusinessEvents (Usuario, Tipo, Entidad, EntidadId, DatosJson) VALUES (@u,@t,@en,@id,@dj);";
            using var cn = new SqlConnection(_cs);
            using var cmd = new SqlCommand(sql, cn);
